...
pygame.init()
pygame.display.set_caption("BS_simulator <Best_effort>")
...
while running:
  ...
  for i in range(len(carsprites)):
    ...
    DB_MAX=carsprites[i].DB
    strongest_bs=carsprites[i].BS
    for j in range(len(BS_sprites)):
    ...
    draw_line(screen,(0,0,0),(carsprites[i].rect.centerx,carsprites[i].rect.centery),(BS_sprites[strongest_bs].rect.centerx,BS_sprites[strongest_bs].rect.centery),2)
  ...
  draw_text(screen,"Total car:"+str(len(carsprites)),30,(BLOCK_SIZE+10)*10+170,HEIGHT-50,BLACK)
  all_sprites.update()
