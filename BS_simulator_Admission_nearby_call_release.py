...
pygame.init()
pygame.display.set_caption("BS_simulator <Admission_nearby_call_release>")
...
def car_create(car):
  ...
  if DB_nearby_MAX==-1 or strongest_nearby_bs==-1:
    ...
    BS_sprites[strongest_nearby_bs].load_car+=1
    car.BS=strongest_nearby_bs
    car.DB=DB_nearby_MAX
...
while running:
  ...
  for i in range(len(carsprites)):
      ...
      for j in range(len(BS_sprites)):
        ...
        if dis<=(radius/Distance_scaler):
          if DB>DB_nearby_MAX:
            ...
            strongest_nearby_bs=j
      if DB_nearby_MAX==-1 or strongest_nearby_bs==-1:
        if strongest_bs!=carsprites[i].BS:
          switch_count+=1
        BS_sprites[strongest_bs].load_car+=1
        carsprites[i].BS=strongest_bs
        carsprites[i].DB=DB_MAX
      else:
        if strongest_nearby_bs!=carsprites[i].BS:
          switch_count+=1
        BS_sprites[strongest_nearby_bs].load_car+=1
        carsprites[i].BS=strongest_nearby_bs
        carsprites[i].DB=DB_nearby_MAX
    ...
    #draw_car_text(screen,str(int(carsprites[i].sec/60))+" min",15,carsprites[i].rect.centerx+20,carsprites[i].rect.centery)
